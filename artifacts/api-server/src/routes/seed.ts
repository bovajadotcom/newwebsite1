...
router.post("/seed", async (req, res): Promise<void> => {
  ...
  if (existingVehicles.length === 0) {
    ...
    results.push("Inserted 25 vehicles");
  } else {
    results.push(`Vehicles already present (${existingVehicles.length}), skipping`);
  }

  // Popular vehicles
  const existingPopular = await db.select().from(popularVehiclesTable);
  if (existingPopular.length === 0) {
    const popular: InsertPopularVehicle[] = [
    ...
    await db.insert(popularVehiclesTable).values(popular);
    results.push("Inserted 10 popular vehicles");
  } else {
    results.push(`Popular vehicles already present (${existingPopular.length}), skipping`);
  }
